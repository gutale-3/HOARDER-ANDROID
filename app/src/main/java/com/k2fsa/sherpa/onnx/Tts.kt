...
//
// This is the official sherpa-onnx Kotlin API wrapper, vendored into the app so
// that the offline neural TTS (Piper) engine can be used. The native
// implementation lives in `libsherpa-onnx-jni.so`, which is supplied by the
// prebuilt sherpa-onnx Android AAR (see app/libs/README-sherpa.md). When the
// native library is NOT present, constructing `OfflineTts` throws at runtime and
// the app transparently falls back to the system TextToSpeech engine.
//
// Source: https://github.com/k2-fsa/sherpa-onnx (sherpa-onnx/kotlin-api/Tts.kt)
